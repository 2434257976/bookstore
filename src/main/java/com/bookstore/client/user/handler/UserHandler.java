...
public class UserHandler {
    ...
    public String register(User user, String checkCode, HttpSession session, HttpServletRequest request) throws MessagingException {
    ...
    private void addCookie(String autologin, User user, HttpServletRequest request, HttpServletResponse response) {
        ...
        //把定义好的cookie响应回客户端
        response.addCookie(cookie1);
        if ("1".equals(autologin)){//定义cookie对象：记住密码
    ...
    private User autologin(HttpServletRequest request) {
       //
        ...
        //获取全部的cookie信息保存到数组cookies中
        Cookie[] cookies= request.getCookies();
        //循环cookies数组
        for (Cookie cookie:cookies){
            //对比找出cookies中保存的bookstore_username的名称
            if ("bookstore_username".equals(cookie.getName())){
                //把对应bookstore_username的cookie值赋给上面定义的username
              username=  cookie.getValue(); }
            //对比找出cookies中保存的bookstore_password的名称
            if ("bookstore_password".equals(cookie.getName())){
                //把对应bookstore_password的cookie值赋给上面定义的password
